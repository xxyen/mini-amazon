...
def gen_orders(num_users, num_orders):
    with open('Orders.csv', 'w') as f:
        ...
        for o_orderKey in range(num_orders):
            ...
            o_shipDate = fake.date_time_between(start_date='-1y', end_date='now')
            o_diliveryDate = fake.date_time_between(start_date='-1y', end_date='now')
            
            
            writer.writerow([o_orderKey, o_date, o_uid, o_shipDate, o_diliveryDate])
...
def gen_lineItems(num_orders, product_sellers):
    num_lineitems = 0
    with open('LineItems.csv', 'w') as f:
        ...
        for order_key in range(num_orders):
            ...
            for _ in range(num_items):
                ...
                num_lineitems += 1
                writer.writerow([order_key, ps_sid, ps_pid, li_amount, li_number, li_fulfilment])
...
gen_orders(num_users, num_orders)
product_sellers = gen_productSellers(num_products, num_users)
gen_lineItems(num_orders, product_sellers)
